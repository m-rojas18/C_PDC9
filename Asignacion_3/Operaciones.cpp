...
void Operaciones::escribir_o_leer(fstream &fcont, bool Escribir){
    for (int i = 0; i < 6; ++i)
        if (Escribir) {
...
void Operaciones::leer_el_3(fstream &fcont, Contacto &cont){
    fcont.seekp(3*sizeof(Contacto));
    fcont.read(reinterpret_cast< char *>(&cont), sizeof(Contacto));
    cout << cont.obtenerId() << ":" << cont.obtenerNombre() << endl;
}

void Operaciones::escribir_el_3(fstream &fcont, Contacto &cont) {
    fcont.seekp(3*sizeof(Contacto));
...
void Operaciones::cambios(fstream &fcont) {
    Contacto cont;
